...
**?**:
```typescript
// src/usecases/geocode/GeocodeUsecase.ts
...
- **????**: `src/usecases/autocomplete/AutocompleteUsecase.ts`
...
- **????**: `src/usecases/directions/DirectionsUsecase.ts`
...
1. **??????????**: `src/usecases/[feature]/[Feature]Usecase.ts`
   - ??????????
